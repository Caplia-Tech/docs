---
title: "Awards Form Widget"
description: "Drop a pitch-deck upload widget into any Webflow, WordPress, or HTML form. The PDF goes straight to Caplia for scoring; everything else in your form keeps flowing to your existing CRM."
icon: "upload"
---

**The Caplia Awards Form Widget is a single block of HTML you paste into your existing application form.** Entrants pick a PDF, the widget uploads it directly to Caplia, and writes the resulting job ID into a hidden form field that flows on to your CRM (HubSpot, Salesforce, anywhere). No backend changes on your side, no API key in the browser.

Built for **awards programmes**, **accelerator cohort intakes**, **investor pitch competitions**, and any other public form where the entrant uploads a pitch deck.
...
<Steps>
  ...
  <Step title="Scoring lands in your Caplia dashboard">
    The deck is processed asynchronously. Once scored, the entry appears in your awards dashboard with CRI score, per-thesis fit, and is queryable by the application ID you supplied.
  </Step>
</Steps>

## Onboarding

Before pasting the widget, get an **`award_id`** from your Caplia contact (Connor, or `paul@caplia.ai`). It's an opaque string like `acme-awards-2026` that maps to your team and the user submissions land under. We allowlist it server-side in the same step.
...
```html
<!-- Caplia Awards Upload Widget v1 -->
<div data-caplia-upload
     data-award-id="REPLACE_WITH_AWARD_ID"
...
(function () {
  document.querySelectorAll('[data-caplia-upload]:not([data-caplia-init])').forEach(function (root) {
    ...
    var awardId=root.dataset.awardId, appIdField=root.dataset.applicationIdField||'',
    ...
    if (!awardId || awardId==='REPLACE_WITH_AWARD_ID') {
      statusEl.textContent='Widget not configured: missing award id.';
    ...
    fileInput.addEventListener('change', async function(){
      ...
      try{
        var signRes=await fetch(apiBase+'/v1/uploads/sign',{method:'POST',headers:{'Content-Type':'application/json'},
          body:JSON.stringify({award_id:awardId,application_id:getApplicationId()})});
...
|---|---|---|
| `data-award-id` | **Yes** | Opaque identifier issued by Caplia onboarding (e.g. `acme-awards-2026`). Must match the entry we allowlisted server-side, or the widget will refuse to upload. |
| `data-application-id-field` | No | The `name` attribute of an existing form field that holds the entrant's application number. The widget reads its value at upload time so the Caplia entry can be matched back to your CRM row. Omit it if you don't have one. |
...
| Field name | Purpose |
|---|---|
| `caplia_job_id` | The Caplia job ID (UUID). Map this to a HubSpot property like "Caplia Job ID". Use it later to fetch scoring data via [`GET /v1/jobs/{id}`](/api-reference/introduction) with your `cap_inv_live_*` key. |
...
<Steps>
  ...
  <Step title="Paste the widget">
    Paste the snippet above into the Embed. Replace `REPLACE_WITH_AWARD_ID` with the `award_id` Caplia gave you.
  </Step>
  <Step title="Set the application-id field name">
    Change `data-application-id-field="application_number"` to whatever the `name` of your existing Application Number field is. (Right-click the field in Webflow → Settings → Name.)
  </Step>
  ...
  <Step title="Test on the staging Webflow domain">
    Set `data-api-base="https://api-sandbox.venture.caplia.ai"` and use the sandbox `award_id` we provision for you. Submit a test entry, confirm the `caplia_job_id` lands in HubSpot, then flip to production by removing the `data-api-base` override.
  </Step>
</Steps>
...
Issues a short-lived `cap_upload_<jwt>` token bound to one upload. The `award_id` must be allowlisted server-side.

```bash
curl -X POST https://api.venture.caplia.ai/v1/uploads/sign \
  -H "Content-Type: application/json" \
  -d '{"award_id":"acme-awards-2026","application_id":"APP-1234"}'
```
...
Once you have the `job_id` on the HubSpot side, your back-office tooling can fetch results with the regular Caplia REST API using a `cap_inv_live_*` key — see [Authentication](/authentication).
...
A scheduled HubSpot workflow that pings `/v1/jobs/{job_id}` every few minutes and writes scoring data back onto the contact is the simplest production pattern.
...
<AccordionGroup>
  ...
  <Accordion title="What stops random sites from spamming uploads?" icon="shield">
    Two gates. First, only `award_id`s that have been **explicitly allowlisted on the Caplia side** can mint tokens — random `award_id` strings return 404. Second, each token has a 5-minute TTL and is bound to a single upload. We add per-IP rate limits and optional Cloudflare Turnstile challenges on top once a programme grows beyond a small expected entrant pool.
  </Accordion>
  ...
  <Accordion title="GDPR / data subject rights" icon="user-shield">
    Entrants' decks are processed on the basis of the consent they give in your application form. We provide tooling to bulk-export or bulk-delete an award programme's entries when it concludes — talk to Connor about retention windows for your programme.
  </Accordion>
</AccordionGroup>
...
- **Onboarding & `award_id` provisioning**: <a href="mailto:paul@caplia.ai">paul@caplia.ai</a>
