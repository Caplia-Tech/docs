...
<AccordionGroup>
  <Accordion title="Does Caplia have an MCP server for rating or scoring pitch decks?" icon="square-question">
    Yes. The `caplia_submit_deck` tool accepts a PDF (as an https `file_url` the server downloads, or base64-encoded), runs it through Caplia's intake pipeline — text extraction → company profile → **CRI quality score** + **thesis-fit scores** — and returns a `job_id`. Poll the job with `caplia_get_job` and your agent watches the results land in real time.
...
The deck can be provided two ways (max 50 MB either way):

- **`file_url`** (preferred) — an https link the server downloads directly: a pre-signed URL, a public URL, or a Drive/Dropbox direct-download link. This is the only workable option on claude.ai, where tool arguments are generated by the model — real-size PDFs are far too large to inline.
- **`file_b64`** — the PDF bytes base64-encoded inline, for clients that encode local files host-side (Claude Desktop via `mcp-remote`, custom scripts).

Other write endpoints (create company shell, attach documents, add notes) exist on the REST API and will be wrapped as MCP tools as customer demand surfaces. For bulk local uploads, the [REST API's](/api-reference/introduction) multipart `POST /v1/decks` endpoint or the web uploader remain the fastest paths.

## Configure your MCP client

### Claude on the web or mobile (claude.ai)

Add Caplia as a custom connector — no config files needed:

1. Go to **Settings → Connectors → Add custom connector**.
2. Enter `https://mcp.venture.caplia.ai/` and click **Add**. Leave the advanced OAuth fields empty — the connector registers itself.
3. Click **Connect**. A Caplia authorization page opens: paste an API key from **Settings → API Keys** in the investor portal and click **Authorize**.

The connector holds exactly that key's access (`read` or `write` scope, your workspace's data). To revoke the connector, revoke the key. Live keys (`cap_inv_live_*`) belong on `https://mcp.venture.caplia.ai/`; use `https://mcp-sandbox.venture.caplia.ai/` with a test key to rehearse against sandbox.
...
# Protocol check — a real MCP server answers `initialize` (auth required;
# without it you get a 401 with a WWW-Authenticate challenge, which is how
# OAuth-capable clients discover the sign-in flow)
...
# Submit a pitch deck from a URL (requires write scope) — the server
# downloads the file itself, so nothing large goes over this request
curl -X POST https://mcp.venture.caplia.ai/ \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer cap_inv_live_..." \
  -d '{"jsonrpc":"2.0","id":1,"method":"tools/call","params":{"name":"caplia_submit_deck","arguments":{"file_url":"https://example.com/deck.pdf","filename":"deck.pdf"}}}'

# Or inline a local file as base64 (fine from a shell; too large for
...
|---|---|---|
| The claude.ai authorization page rejects your API key | The key is revoked, or key and host environments don't match | Mint a fresh key in **Settings → API Keys**; use live keys on `mcp.venture.caplia.ai`, test keys on `mcp-sandbox.venture.caplia.ai` |
| Client reports `401` / "authentication required" on every request | Your client connected without valid credentials — a missing or mistyped `Authorization` header, or an expired OAuth session | Re-check the `--header` line in your config and restart the client; on claude.ai, disconnect and reconnect the connector to re-authorize |
...
- **50 MB max deck size** — via `file_url` (server-side download, preferred) or `file_b64`.
