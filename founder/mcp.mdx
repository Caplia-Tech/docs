...
<Steps>
  ...
  <Step title="Note the server URL">
    Sandbox: `https://mcp-sandbox.venture.caplia.ai/` for `cap_fnd_test_*` keys. Production: `https://mcp.venture.caplia.ai/` for `cap_fnd_live_*` keys. The environments are strict — a test key is rejected by the production host and vice versa. The examples below use the sandbox.
  </Step>
...
<Tabs>
  <Tab title="Claude (web & mobile)">
    Add Caplia as a custom connector on claude.ai — no config files or Node.js needed:

    1. Go to **Settings → Connectors → Add custom connector**.
    2. Enter `https://mcp-sandbox.venture.caplia.ai/` (or `https://mcp.venture.caplia.ai/` with a `cap_fnd_live_*` key) and click **Add**. Leave the advanced OAuth fields empty — the connector registers itself.
    3. Click **Connect**. A Caplia authorization page opens: paste your `cap_fnd_*` key and click **Authorize**.

    The connector holds exactly that key's access — revoke the key in the founder portal to disconnect it. Remember the hosts are strict: test keys on the sandbox host, live keys on the production host.
  </Tab>
...
# unauthenticated requests get a 401 WWW-Authenticate challenge)
...
|---|---|---|
| The claude.ai authorization page rejects your key | The key is revoked, or key and host environments don't match | Mint a fresh `cap_fnd_*` key; test keys go with the sandbox host, live keys with the production host |
...
| `401` / invalid key, even though the key looks right | Key and host environments don't match | `cap_fnd_test_*` keys only work on `mcp-sandbox.venture.caplia.ai`; `cap_fnd_live_*` keys only on `mcp.venture.caplia.ai` |
| Investor tools appear instead of the founder tools | The key isn't a `cap_fnd_*` key | Mint a founder key in the founder portal (see [Authentication](/founder/authentication)) |
...
| Deck-loop tools fail with a permissions error | Key has `read` scope only | Mint a `write`-scoped `cap_fnd_*` key |
...
- **50 MB max** per deck or document — provide files as an https `file_url` the server downloads directly (preferred; required on claude.ai, where inlined base64 can't carry real-size files), or as `file_b64` from clients that encode host-side.
