...
<Steps>
  <Step title="Get a founder key">
    Mint a `cap_fnd_*` key in the founder portal (see [Authentication](/founder/authentication)). Give it the **`write`** scope so the assistant can submit and publish decks. During the preview, that's a `cap_fnd_test_*` key.
  </Step>
