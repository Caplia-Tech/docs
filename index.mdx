...
<Columns cols={2}>
  ...
  <Card title="Use it from AI agents" icon="plug" href="/mcp">
...
- **Production.** `api.venture.caplia.ai` — accepts `cap_inv_live_*` keys minted from your live Caplia portal. Your pipeline data.
- **Sandbox.** Use the staging Caplia portal to mint `cap_inv_test_*` keys against a separate staging Supabase project. The same endpoints and tools work, but the data is isolated from production. Ask your team for the staging URL if you don't already have it.
...
- **MCP write tools.** The [MCP server](/mcp) exposes 11 read-only tools today; write tools (create company, submit deck) land in v1.1.
